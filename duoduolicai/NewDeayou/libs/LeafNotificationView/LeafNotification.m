...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.layer.backgroundColor = kCOLOR_R_G_B_A(250, 128, 10, 0.8).CGColor;
        
//        self.layer.cornerRadius = 5.0f;
//        self.layer.opacity = 0.25;
        
        
        self.textLabel = [[UILabel alloc] initWithFrame:CGRectMake(frame.origin.x, frame.origin.y, frame.size.width - 40, frame.size.height)];
...
-(void)setType:(LeafNotificationType)type{
    if(LeafNotificationTypeWarrning == type){
        self.flagImageView.image = [UIImage imageNamed:@""];
...
-(instancetype)initWithController:(UIViewController *)controller text:(NSString *)text{
    if([self initWithFrame:CGRectMake(0, -DEFAULT_HEIGHT, controller.view.bounds.size.width*DEFAULT_RATE_WIDTH, DEFAULT_HEIGHT)]){
        ...
        self.textLabel.center = CGPointMake(self.center.x , self.flagImageView.center.y);
    }
