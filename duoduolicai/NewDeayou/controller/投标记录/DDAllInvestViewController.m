...
- (void)viewDidLoad {
    ...
    _tableView=[[PullingRefreshTableView alloc]initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height- 50 - 64) style:UITableViewStylePlain topTextColor:kBtnColor topBackgroundColor:nil bottomTextColor:kBtnColor bottomBackgroundColor:nil];
...
//        cell.isMonth = [borrow_style isEqualToString:@"month"]?YES:NO;
        ...
        NSString *statusType = [NSString stringWithFormat:@"%@",[dic objectForKey:@"status_type"]];
        if ([statusType isEqualToString:@"end"]) {
            float total=[[dic objectForKey:@"recover_account_yes"] floatValue];
            cell.typenameLabel.text = @"已获本息(元)";
            cell.Mark_earningTotal.text=[NSString stringWithFormat:@"%.2f元",total];
        }else {
             cell.typenameLabel.text = @"待收本息(元)";
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([[dic DYObjectForKey:@"borrow_type"] isEqualToString:@"fragment"]) {
