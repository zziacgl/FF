...
@interface DDAllInvestViewController ()<UITableViewDataSource,UITableViewDelegate,PullingRefreshTableViewDelegate>
...
@implementation DDAllInvestViewController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    _tableView=[[PullingRefreshTableView alloc]initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height- 50 - 64) style:UITableViewStylePlain topTextColor:kMainColor topBackgroundColor:nil bottomTextColor:kMainColor bottomBackgroundColor:nil];
    _tableView.delegate=self;
    _tableView.dataSource=self;
    _tableView.pullingDelegate=self;
    _tableView.backgroundColor = kCOLOR_R_G_B_A(236, 236, 236, 1);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.dataArray.count > 0) {
        ...
        NSString *tender_status =[NSString stringWithFormat:@"%@",[dic objectForKey:@"tender_status"]];
...
//        cell.isMonth = [borrow_style isEqualToString:@"month"]?YES:NO;
        ...
        float total=[[dic objectForKey:@"tender_award_fee"] floatValue]+[[dic objectForKey:@"recover_account_interest"] floatValue] + [[dic objectForKey:@"account"] floatValue];
        ...
        [str addAttribute:NSForegroundColorAttributeName value:kCOLOR_R_G_B_A(253, 83, 83, 1) range:NSMakeRange(3, len)];
