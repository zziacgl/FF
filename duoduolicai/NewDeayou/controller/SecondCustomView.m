...
#import "SecondCustomView.h"

@implementation SecondCustomView
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 1) {
        ...
        cell.textLabel.text = self.str1;
        cell.detailTextLabel.text = [NSString stringWithFormat:@"剩余%@", self.str2];
        cell.detailTextLabel.font = [UIFont systemFontOfSize:14];
    }
    cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    
    //    cell.detailTextLabel.text = [NSString stringWithFormat:@"体验金%@", self.str1];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.delegate &&[self.delegate respondsToSelector:@selector(getAction:)]) {
        [self.delegate getAction:(int)indexPath.row];
