//
//  DDInTransferTableViewController.m
//  NewDeayou
//
...
#import "DDCircle.h"
@interface DDInTransferTableViewController ()<UITableViewDataSource,UITableViewDelegate,PullingRefreshTableViewDelegate>
{
    int page;
    BOOL isViewDidLoad;
}
@property(nonatomic,strong)NSMutableArray *IndataArray;
@property (nonatomic, strong) PullingRefreshTableView *InTFTableView;

@end

@implementation DDInTransferTableViewController
-(void)viewDidAppear:(BOOL)animated{
    ...
    {
        //初始化tableView
        [_InTFTableView launchRefreshing];
        isViewDidLoad=NO;
    }
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.InTFTableView = [[PullingRefreshTableView alloc] initWithFrame:CGRectMake(0, 0, kMainScreenWidth, kMainScreenHeight - 50 - 64) style:UITableViewStylePlain topTextColor:kMainColor topBackgroundColor:nil bottomTextColor:kMainColor bottomBackgroundColor:nil];
    _InTFTableView.delegate = self;
    _InTFTableView.dataSource = self;
    _InTFTableView.pullingDelegate = self;
    _InTFTableView.backgroundColor = kCOLOR_R_G_B_A(236, 236, 236, 1);
    _InTFTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    [self.view addSubview:self.InTFTableView];
//    [_InTFTableView registerClass:[DDBondTableViewCell class] forCellReuseIdentifier:@"in"];
    [_InTFTableView registerClass:[DDNoCardTableViewCell class] forCellReuseIdentifier:@"no"];
     isViewDidLoad = YES;
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    [self.InTFTableView tableViewDidScroll:scrollView];
}

-(void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
    [self.InTFTableView tableViewDidEndDragging:scrollView];
...
-(void)refreshTableView:(id)object
{
    ...
    [self getInRatesData:[object intValue]];
}
//获取数据接口
-(void)getInRatesData:(BOOL)isRefreshing{
    if (isRefreshing)
    {
        page=1;
        ...
        [diyouDic1 insertObject:@"1" forKey:@"transfer_status" atIndex:0];
        ...
        [DYNetWork operationWithDictionary:diyouDic1 completeBlock:^(id object,BOOL success,NSString*error)
         {
             if (_InTFTableView.headerView.isLoading)
             {
                 [_InTFTableView tableViewDidFinishedLoading];
             }
             if (success==YES) {
                 self.IndataArray = [NSMutableArray new];
                 self.IndataArray=[object objectForKey:@"list"];
                 
                 if (self.IndataArray.count==0) {
                     
                 }else{
                     
                     [self.InTFTableView reloadData];
                 }
             }
             else
             {
    ...
    }else
    {
        ...
        [DYNetWork operationWithDictionary:diyouDic1 completeBlock:^(id object,BOOL success,NSString*error)
         {
             if (_InTFTableView.footerView.isLoading)
             {
                 [_InTFTableView tableViewDidFinishedLoading];
             }
             ...
             if (success==YES) {
                 ...
                 if (ary.count>0) {
                     ...
                     {
                         [self.IndataArray addObject:ary[i]];
                     }
                 }else
                 {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
        ...
        NSDictionary *dic=[self.IndataArray objectAtIndex:indexPath.row];
        [cell setAttributeWithDictionary:dic viewController:self];
//        NSLog(@"timeLabel%@", dic);
